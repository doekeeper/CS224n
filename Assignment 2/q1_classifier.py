...
from CS224n.q1_softmax import softmax
from CS224n.q1_softmax import cross_entropy_loss
from CS224n.model import Model
...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        weight = tf.Variable(tf.zeros([self.config.n_features,self.config.n_classes], dtype=tf.float32))
        bias =tf.Variable(tf.zeros([self.config.n_classes], dtype=tf.float32))
        
        pred = softmax(tf.matmul(self.input_placeholder, weight) + bias)
