...
def softmax(x):
    ...
    """
        x is received as 2-D tensor - n_samples rows is representing n samples,
        n_features is representing row vectors for each sample
        
        softmax function is to find softmax value for each row for each sample
        
        1. normalize each row
        
            x_max_in_row = tf.max(x, axis=1).reshape(-1,1)
            normalized_x = x-x_max_in_row
            out = tf.exp(normalized_x)/(tf.sum(tf.exp(normalized_x), axis=1).reshape(-1,1))
    """ 
 

    x_max_in_row = tf.reduce_max(x, axis=1, keep_dims=True)
    normalized_x = x-x_max_in_row
    softmax_x = tf.exp(normalized_x)/tf.reduce_sum(tf.exp(normalized_x), axis=1, keep_dims=True)
...
def cross_entropy_loss(y, yhat):
    ...
    cross_entropy = -tf.reduce_sum(y_float*log_yhat)
